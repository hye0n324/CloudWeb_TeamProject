...
public interface MatePostRepository extends JpaRepository<MatePost, Long> {
    ...
    @Query("SELECT p FROM MatePost p JOIN FETCH p.author " +
           "WHERE (:keyword IS NULL OR p.title LIKE %:keyword% OR p.content LIKE %:keyword%) " +
           "AND (:status IS NULL OR p.status = :status) " +
           "ORDER BY CASE :sort WHEN 'views' THEN p.viewCount ELSE 0 END DESC, p.createdAt DESC")
    List<MatePost> findAllWithFilter(@Param("keyword") String keyword,
                                     @Param("status") RecruitStatus status,
                                     @Param("sort") String sort);

    @Query("SELECT p.title FROM MatePost p WHERE p.title LIKE %:keyword% ORDER BY p.viewCount DESC")
