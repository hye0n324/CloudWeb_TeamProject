...
function ForgotContent() {
    ...
    return (
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="mt-8 text-center text-sm font-medium">
          <Link href="/login" className="text-zinc-400 hover:text-neon-500 transition-colors">
            로그인으로 돌아가기
          </Link>
        </div>
      </div>
  ...
  // 3-2. 성공 결과 단계 (아이디 찾기 결과 / 비밀번호 변경 성공)
  if (step === "success") {
    return (
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 text-center">
        ...
        <AuthButton href="/login" icon={ArrowRight}>
          로그인하러 가기
        </AuthButton>
      </div>
    );
...
export default function ForgotPage() {
  return (
    <Suspense fallback={<div className="text-center py-10 font-medium text-zinc-500">로딩 중...</div>}>
      <ForgotContent />
    </Suspense>
  );
}
